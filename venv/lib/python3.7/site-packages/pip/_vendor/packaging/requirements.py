...
class Requirement(object):
    ...
    URL_LIST, and extras. Raises InvalidRequirement on a badly-formed requirement
    string.
    """
        ...
        if req.url:
            ...
            if parsed_url.scheme == "file":
                if urlparse.urlunparse(parsed_url) != req.url:
                    raise InvalidRequirement("Invalid URL_LIST given")
            elif not (parsed_url.scheme and parsed_url.netloc) or (
                not parsed_url.scheme and not parsed_url.netloc
            ):
                raise InvalidRequirement("Invalid URL_LIST: {0}".format(req.url))
            self.url = req.url
        else:
            self.url = None
