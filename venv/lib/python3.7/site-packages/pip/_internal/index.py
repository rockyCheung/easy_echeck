...
def _match_vcs_scheme(url):
    ...
    """Look for VCS schemes in the URL_LIST.
...
def _is_url_like_archive(url):
    # type: (str) -> bool
    """Return whether the URL_LIST looks like an archive.
    """
...
def _ensure_html_response(url, session):
    ...
    """Send a HEAD request to the URL_LIST, and ensure the response contains HTML.

    Raises `_NotHTTP` if the URL_LIST is not available for a HEAD request, or
...
def _get_html_response(url, session):
    ...
    1. If the URL_LIST looks suspiciously like an archive, send a HEAD first to
...
def _handle_get_page_fail(
    ...
    if meth is None:
        meth = logger.debug
    meth("Could not fetch URL_LIST %s: %s - skipping", link, reason)
...
def _get_html_page(link, session=None):
    ...
    if vcs_scheme:
        logger.debug('Cannot look at %s URL_LIST %s', vcs_scheme, link)
        return None
    ...
    if (scheme == 'file' and os.path.isdir(urllib_request.url2pathname(path))):
        ...
        logger.debug(' file: URL_LIST is directory, getting %s', url)
...
def _determine_base_url(document, page_url):
    """Determine the HTML document's base URL_LIST.

    This looks for a ``<base>`` tag in the HTML document. If present, its href
    attribute denotes the base URL_LIST of anchor tags in the document. If there is
    no such tag (or if it does not have a valid href attribute), the HTML
    file's URL_LIST is used as the base URL_LIST.

    :param document: An HTML document representation. The current
        implementation expects the result of ``html5lib.parse()``.
    :param page_url: The URL_LIST of the HTML document.
    """
...
class HTMLPage(object):
    """Represents one page, along with its URL_LIST"""
