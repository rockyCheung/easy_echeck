...
def get_file_content(url, comes_from=None, session=None):
    ...
    """Gets the content of a file; it may be a filename, file: URL_LIST, or
    http: URL_LIST.  Returns (location, content).  Content is unicode.
    ...
    if match:
        ...
        if (scheme == 'file' and comes_from and
            ...
            raise InstallationError(
                'Requirements file %s references URL_LIST %s, which is local'
                % (comes_from, url))
        if scheme == 'file':
            path = url.split(':', 1)[1]
            path = path.replace('\\', '/')
...
def is_url(name):
    ...
    """Returns true if the name looks like a URL_LIST"""
    if ':' not in name:
        return False
...
def url_to_path(url):
    # type: (str) -> str
    """
    Convert a file: URL_LIST to a path.
    """
...
def path_to_url(path):
    ...
    """
    Convert a path to a file: URL_LIST.  The path will be made absolute and have
    quoted path parts.
    """
...
def _download_url(
    ...
    logger.debug('Downloading from URL_LIST %s', link)
...
def unpack_file_url(
    ...
    # hash in `hashes` matching: a URL_LIST-based or an option-based
